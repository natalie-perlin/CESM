...
! Post-process input fields
   ...
   case ('T')
      ...
      if (pertlim .ne. 0.0_r8) then
         ...
         do lat = 1, plat
            do i = 1, plon
               ! seed based on global column index plus instance offset so each
               ! ensemble member receives a distinct perturbation pattern
               rndm_seed = i + (lat-1)*nglon + (inst_index-1)*nglon*nglat
